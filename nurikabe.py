...
given_solution = load('/content/water_patterns_5x5_trimmed_format.txt', dot_value=State.SEA)


'''
with open('/content/water_patterns_5x5_trimmed_format.txt', 'r') as f:
    lines = f.readlines()
    for line in lines:
        temp = []
        line = line.replace(' ', '')
    ...
    def solve_loop(self):
        while True:
            ...
            if operations == 0:
                ...
                if valid:
                    if self.validate():
                        print(self.puzzle, "progress puzzle")
                        if (self.puzzle == given_solution).all():
                            #keep searching for solution, as that solution is already given. we are looking for if the puzzle has more than one solution
                            self.solved = False
                            print("found already given solution")
...
def main():
    ...
    if success:
        logging.info("Puzzle has more than one solution.")
    else:
        full_puzzles = []
